...
// This is the entry point for FreeRTOS which is the task schedular we will
// be using in order to get fast data throughput. We will need to set up wifi
// and http request module at the top level for global use. These are a main
// mode of external communication. We must also support WPA2 enterprise auth
// which can be a pain as it requires cert management and submission of 
// credentials to the local auth server. I expect this to be an inordinate
// amount of code which I am not excited to attempt
void app_main()
{
